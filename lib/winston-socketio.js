...
var SocketIO = exports.SocketIO = function(options) {
    ...
    this.channel = options.channel || "log";
...
SocketIO.prototype.log = function(level, msg, meta, callback) {
    ...
    var do_log = function() {
        ...
        if (_this._state === "open") {
            _this.socket.emit(_this.channel, _this.log_format(level, msg, meta));
            callback();
        }
    }
...
SocketIO.prototype.open = function(callback) {
    ...
    if (this.path) {
        this.socket = io(this.host + ":" + this.port + "/" + this.path, { secure: this.secure, reconnect: this.reconnect });
    } else {
